...
STARTGETTINGSHAPES = "^StartGettingShapes mass=(\S+) xcg=(\S+) vair=(\S+) psi=(\S+) theta=(\S+) phi=(\S+) alpha=(\S+) betha=(\S+) a0=(\S+) trim=(\S+) dl=(\S+) dm=(\S+) dn=(\S+)"
STARTCOMPUTING = "^StartComputation mass=(\S+) xcg=(\S+) vair=(\S+) psi=(\S+) theta=(\S+) phi=(\S+) alpha=(\S+) betha=(\S+) a0=(\S+) trim=(\S+) dl=(\S+) dm=(\S+) dn=(\S+) dx=(\S+) p=(\S+) q=(\S+) r=(\S+)"
...
def onStartGettingShape(client, *args):
    mass,xcg,vair,psi,theta,phi,alpha,betha,a0,trim,dl,dm,dn = [float(arg) for arg in args]
...
def onStartComputing(agent, *args):
    ...
    mass,xcg,vair,psi,theta,phi,alpha,betha,a0,trim,dl,dm,dn,dx,p,q,r = [float(arg) for arg in args]
    kscale = 1.5e-5
